...
pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool>  hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
   ...
   //iterator made to find the element 
   typename list <pair<const Key, Value>>::iterator itr = findElement(x.first, bucket);
   // if element not found, will insert it
   if (findElement(x.first,bucket) == elems_[bucket].end()) {
     //modify size
     ++size_;
     elems_[bucket].push_back(x);
     typename list <pair<const Key, Value>>::iterator itr = findElement(x.first, bucket);
     return make_pair(&(*itr), true);
   }else {
     //doesn't insert if element is already inside 
...
hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
    ...
    size_t bucketOne = hash_.hash(x);
    auto itr = findElement(x, bucketOne);

    if (itr != elems_[bucketOne].end()) {    // the element exists, erase it
        size_t bucketIncrement;


        auto itrTwo = elems_[bucketOne].erase(itr);
        --size_;
        if (itrTwo == elems_[bucketOne].end()) {
            bucketIncrement = bucketOne + 1;

            while (bucketIncrement < elems_.size()) {
                if (!elems_[bucketIncrement].empty()) {
                    return make_pair(&(*elems_[bucketIncrement].begin()), true);
                }
                ++bucketIncrement;
            }
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
    ...
    Element* finder = find(x);
    if (finder == nullptr) { // if key not found, create new element with empty value
        auto inserter = insert(make_pair(x, Value()));
...
template <typename Key, typename Value, typename Compare, typename Hash> 
void hashmap <Key, Value, Compare, Hash>::rehash(size_t sizer) {
    vector<Element> hasher;
    //holds number of buckets
    auto holder = hash_.numBuckets();
    if (sizer <= holder) {
        return;
    }

    //changes size of the bucket
    hash_ = DefaultHash<Key>(sizer);
    holder = hash_.numBuckets();
    for (auto& itr : elems_) {
        for (auto& itrTwo : itr) {
            hasher.push_back(itrTwo);
        }
    }

    elems_.clear();
    elems_ = vector<list<Element>> (holder);
    for (auto& itrThree : hasher) {
        size_t a = hash_.hash(itrThree.first);
        elems_[a].push_back(itrThree);
    }
}
